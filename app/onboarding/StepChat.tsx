...
import { callOnboardingChat, upsertOnboardingState } from "../../lib/onboarding";
...
export default function StepChat({
  userId, userName, educationLevel,
  chatHistory, onChatHistoryUpdate, onComplete
}: Props) {
  ...
  const sendToAI = async (history: ChatMessage[], userText?: string) => {
    ...
    try {
      ...
      setBubbles(updatedHistory.map((m) => ({ role: m.role, text: m.parts[0].text })));
  ...
  return (
    ...
    >
      ...
      >
        {bubbles.map((b, i) => (
          <View key={i} style={[styles.bubble, b.role === 'user' ? styles.bubbleUser : styles.bubbleAI]}>
            <Text style={[styles.bubbleText, b.role === 'user' ? styles.bubbleTextUser : styles.bubbleTextAI]}>
              {b.text}
            </Text>
          </View>
        ))}
      ...
      <View style={styles.inputRow}>
        ...
        />
        <Pressable style={[styles.sendBtn, (!input.trim() || loading) && styles.sendBtnDisabled]} onPress={handleSend}>
...
const styles = StyleSheet.create({
  ...
  sendBtn: {
    backgroundColor: '#BFFF00', width: 44, height: 44,
